...
public class DeskUtils {
	...
	public static List<DeskDto> fromDeskListToDeskDtoList(List<Desk> seatList) {
		return seatList.stream().map(t -> new DeskDto(t.getDeskId().getDeskNo(), t.getDeskId().getOfficeId(), t.getXPos(), t.getYPos(), t.isCanBeReserved(), t.getIsAvailableForSelectedDays())).collect(Collectors.toList());
 	}
