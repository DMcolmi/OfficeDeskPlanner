...
public interface DeskRepository extends JpaRepository<Desk, DeskId> {
	...
	@Query(value = "select d.*, null as is_reserved from DESKS d where office_id = :office", nativeQuery = true)
	...
	@Query(value = "select distinct d.desk_no, d.office_id, can_be_reserved, x_pos, y_pos,\r\n"
			+ "case when r.is_reserved is not null then r.is_reserved else false end as is_reserved \r\n"
			+ "from desks d  \r\n"
			+ "left join reservations r on d.office_id = r.office_id and d.desk_no = r.desk_no and r.booking_date in :selectedDays \r\n"
